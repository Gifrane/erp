<template>
    <div :id="dropdownId" class="dropdown">
        <div class="dropdown-header" @click="displayDropdownAction()">
            <div class="header-title placeholder" v-if="selected_length === 0">{{ $t('pages.crm_contacts.groups_list') }}</div>
        ...
        <div class="dropdown-menu w-100" v-click-outside="outside" :class="{ show: displayDropdown }">
            <template v-if="groups.length > 0">
                <div class="menu-search" v-if="groups.length > 4">
                    ...
                    <template v-for="g in groups">
                        <div class="menu-list-selectable-item" :key="g.id" v-if="!search || regexSearch(g.name)">
                            <div class="selectable-item-checkbox">
                                <input type="checkbox" v-model="$parent.add.groups" :value="g.id"
                                    @change="updateSelection(g.id, g.name)" :id="'add-group-'+g.id" />
                            </div>

                            <label class="selectable-item-label" :for="'add-group-'+g.id">{{ g.name }}</label>
                        </div>
                    </template>
                </div>
            </template>

            <div class="menu-empty" v-else>{{ $t('pages.crm_contacts.no_group_found') }}</div>
        </div>
    </div>
</template>
...
export default {
    ...
    data: function () {
        return {
            dropdownId: 'dropdown-add-contact-groups',
    ...
    methods: {
        deleteSelection: function (id) {
            ...
            this.$parent.add.groups = this.$parent.add.groups.filter((g) => g !== id)
        },
