<template>
    <div class="modal-wrapper" v-if="visible">
        <div class="modal" v-click-outside="outside">
            <div class="modal-header">
                <div class="modal-header-title">
                    <span>{{ $t('tags.add_new_tag') }}</span>
                </div>
            ...
            <div class="modal-body">
                <input type="text" v-model="tag" :placeholder="$t('tags.enter_new_tag')" class="input w-100" />
            </div>
...
export default {
    ...
    created: function () {
        EventBus.on('visible_add_tag', () => this.visible = true)
    },
    methods: {
        addTag: function () {
            axios.post('/crm/contacts/'+this.contact_id+'/edit/add/tag', { tag: this.tag })
                .then((res) => {
                    EventBus.emit('add_new_tag', res.data.data)
