<template>
    <div class="companies">
        <div class="tabs">
            ...
            <router-link to="/crm/contacts">{{ $t('pages.crm_companies.contacts') }}</router-link>
            <router-link to="/crm/groups">{{ $t('pages.crm_companies.groups') }}</router-link>
            <router-link to="/crm/companies" class="active">{{ $t('pages.crm_companies.companies') }}</router-link>
            <router-link to="/crm/reviews">{{ $t('pages.crm_companies.reviews') }}</router-link>
        </div>
        ...
        <div class="companies-list">
            <div class="list-actions btn-align">
                <button class="btn btn-outline-primary mr-2" @click="visibleAdd = true">
                    ...
                    <span>{{ $t('pages.crm_companies.add_company') }}</span>
                </button>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>{{ $t('country') }}</th>
                            <th>{{ $t('city') }}</th>
                            <th>{{ $t('pages.crm_companies.contacts_count') }}</th>
                            <th>{{ $t('pages.crm_companies.sales_count') }}</th>
                            <th>{{ $t('website') }}</th>
                    ...
                    <tbody>
                        <tr v-for="c in companies" :key="c.id">
                            ...
                            <td class="td-content">{{ c.name }}</td>
                            <td class="td-content">{{ c.country.name }}</td>
                            <td class="td-content">{{ c.city }}</td>
                            <td class="td-content">{{ c.contacts_count }}</td>
                            <td class="td-content">{{ c.sales_count }}</td>
                            <td class="td-content"><a :href="c.website">{{ c.website }}</a></td>
                            <td class="td-actions">
                                <router-link :to="'/crm/companies/'+c.id">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <svg @click="deleteCompany(c.id)" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6 7H5V20C5 20.5304 5.21071 21.0391 5.58579 21.4142C5.96086 21.7893 6.46957 22 7 22H17C17.5304 22 18.0391 21.7893 18.4142 21.4142C18.7893 21.0391 19 20.5304 19 20V7H6ZM16.618 4L15 2H9L7.382 4H3V6H21V4H16.618Z" fill="black"/>
                                </svg>
                                ...
                                <!--<svg v-tippy="$t('bookmark.add')" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
import SidebarAddCompany from './companies/SidebarAddCompany.vue'

export default {
    components: { SidebarAddCompany },
    mixins: [ bookmark ],
    data: function () {
        return {
            order: { column: 'name', order: 'asc' },
            pagination: { page: 1, per_page: 10 },
            method_search: 'getCompanies',
            total: 0,
    ...
    methods: {
        deleteCompany: function (company) {
            axios.post('/crm/companies/'+company+'/destroy')
                .then(() => {
                    this.getCompanies()
                })
                .catch((e) => {
        ...
        getCompanies: function () {
            axios.post('/crm/companies/search', {
...
@import '../../../../sass/vuejs/crm/companies/companies.sass'
</style>