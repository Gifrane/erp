<template>
    <div :id="dropdownId" class="dropdown" style="width: 200px">
        <div class="dropdown-header" @click="displayDropdownAction()">
            <div class="header-title placeholder" v-if="selected === null">{{ $t('days_list') }}</div>
...
export default {
    ...
    data: function () {
        return {
            dropdownId: 'dropdown-update-day-send-recap',
    ...
    created: function () {
        this.options = [ 'monday', 'Tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday' ].map((o) => {
            return { code: o, label: this.$t('days.'+o) }
        })
    },
    methods: {
        select: function (o) {
