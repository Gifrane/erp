    ...
    <div class="sidebar-wrapper" v-if="$parent.visibleAdd">
        <div class="sidebar" role="complementary" v-click-outside="outside">
            <div class="sidebar-header">
                <div class="sh-title">{{ $t('pages.admin_api_keys.generate_new_keys') }}</div>
            ...
            <div class="sidebar-body">
                ...
                <div class="grid mt-2">
                    ...
                    <div class="col-8"><multi-select-modules /></div>
                </div>

                <div class="grid mt-2">
                    <div class="col-4">{{ $t('pages.admin_api_keys.expiration_date') }}</div>
                    <div class="col-8"><Calendar class="w-100" v-model="add.expired_at" /></div>
                </div>

                <button class="btn btn-success" @click="addKey()">
...
import MultiSelectModules from './add_keys/MultiSelectModules.vue'
...
export default {
    ...
    data: function () {
        return {
            add: { name: null, modules: [], expired_at: null }
        }
    },
    methods: {
        addKey: function () {
            axios.post('/admin/api_keys/add', this.add)
                .then(() => {
