                ...
                <template v-for="m in modules">
                    <div class="menu-list-selectable-item" :key="m.code" v-if="!search || regexSearch(m.name)">
                        <div class="selectable-item-checkbox">
                            <input type="checkbox" v-model="$parent.add.modules" :value="m.code"
                                @change="updateSelection(m.code, m.name)" :id="'add-module-'+m.code" />
                        </div>

                        <label class="selectable-item-label" :for="'add-module-'+m.code">{{ m.name }}</label>
                    </div>
                </template>
            </div>
        </div>
...
export default {
    ...
    data: function () {
        return {
            dropdownId: 'dropdown-add-keys-modules',
    ...
    methods: {
        deleteSelection: function (code) {
            ...
            this.$parent.add.modules = this.$parent.add.modules.filter((m) => m !== code)
        },
