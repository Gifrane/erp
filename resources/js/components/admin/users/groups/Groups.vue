<template>
    <div class="groups">
        <div class="tabs">
            <router-link to="/admin/users">{{ $t('pages.admin_users_groups.users') }}</router-link>
            <router-link to="/admin/users/groups" class="active">{{ $t('pages.admin_users_groups.groups') }}</router-link>
            <router-link to="/admin/users/companies">{{ $t('pages.admin_users_groups.companies') }}</router-link>
        </div>
        ...
        <div class="groups-list">
            <div class="list-actions">
                <button class="btn btn-outline-primary mr-2" @click="visibleAdd = true">
                    ...
                    <span>{{ $t('pages.admin_users_groups.add_group') }}</span>
                </button>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="th-with-icon">{{ $t('pages.admin_users_groups.users_count') }}</th>
                    ...
                    <tbody>
                        <tr v-for="g in groups" :key="g.id">
                            ...
                            <td class="td-content">{{ g.users_count }}</td>
                            <td class="td-actions">
                                <router-link :to="'/admin/users/groups/'+g.id">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
    ...
    methods: {
        deleteGroup: function (group) {
            axios.post('/admin/users/groups/'+group+'/destroy')
                .then(() => {
                    this.getGroups()
                })
                .catch((e) => {
        ...
        getGroups: function () {
            axios.post('/admin/users/groups/search', {
...
@import '../../../../../sass/vuejs/admin/users/groups/groups.sass'
</style>