<template>
    <div class="companies">
        <div class="tabs">
            <router-link to="/admin/users">{{ $t('pages.admin_users_companies.users') }}</router-link>
            <router-link to="/admin/users/groups">{{ $t('pages.admin_users_companies.groups') }}</router-link>
            <router-link to="/admin/users/companies" class="active">{{ $t('pages.admin_users_companies.companies') }}</router-link>
        </div>
        ...
        <div class="companies-list">
            <div class="list-actions">
                <button class="btn btn-outline-primary mr-2" @click="visibleAdd = true">
                    ...
                    <span>{{ $t('pages.admin_users_companies.add_company') }}</span>
                </button>
            </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="th-with-icon">{{ $t('pages.admin_users_companies.users_count') }}</th>
                    ...
                    <tbody>
                        <tr v-for="c in companies" :key="c.id">
                            ...
                            <td class="td-content">{{ c.users_count }}</td>
                            <td class="td-actions">
                                <router-link :to="'/admin/users/companies/'+c.id">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
    ...
    methods: {
        getCompanies: function () {
            axios.post('/admin/users/companies/search', {
...
@import '../../../../../sass/vuejs/admin/users/companies/companies.sass'
</style>