    ...
    <div class="sidebar-wrapper" v-if="$parent.visibleAdd">
        <div class="sidebar" role="complementary" v-click-outside="outside">
            <div class="sidebar-header">
                <div class="sh-title">{{ $t('pages.products.add_product') }}</div>
            ...
            <div class="sidebar-body">
                ...
                <div class="grid mt-2">
                    <div class="col-4">{{ $t('pages.products.price_before_tax') }}</div>
                    <div class="col-8"><input type="number" v-model="add.price_before_tax" class="input w-100" /></div>
                </div>

                <div class="grid mt-2">
                    <div class="col-4">{{ $t('pages.products.price_with_tax') }}</div>
                    <div class="col-8"><input type="number" v-model="add.price_with_tax" class="input w-100" /></div>
                </div>

                <div class="grid mt-2">
                    <div class="col-4">{{ $t('pages.products.vat') }}</div>
                    <div class="col-8"><input type="number" v-model="add.tva" class="input w-100" /></div>
                </div>
...
export default {
    ...
    data: function () {
        return {
            add: { name: null, price_before_tax: 0, price_with_tax: 0, tva: 0 }
        }
    },
    methods: {
        addProduct: function () {
            axios.post('/product/add', this.add)
                .then((res) => {
                    this.$router.push('/product/'+res.data.data)
                })
                .catch((e) => {
