<template>
    <div class="provider-block-wrapper">
        <template v-if="display === 'select'">
            <div class="mt-3">
                <div class="provider-block-select-header">{{ $t('pages.products_details_spare_parts.provider') }}</div>

                <div class="col-12">
                    <dropdown-provider :providers="providers" />
                </div>
            </div>
        </template>
        ...
        <template v-else-if="display === 'create'">
            <div class="mt-3">
                <div class="provider-block-name-header">{{ $t('pages.products_details_spare_parts.register_provider') }}</div>

                <div class="col-12">
                    <input type="text" class="input w-100" v-model="$parent.add.provider.name" />
                </div>
            </div>
        </template>

        <div class="provider-block-menu">
            <div class="menu-buttons">
                ...
                <div class="menu-buttons-menu" :class="{ 'd-none': !display_menu }">
                    <div class="buttons-menu-link" @click="access('select')">{{ $t('pages.products_details_spare_parts.select_provider') }}</div>
                    <div class="buttons-menu-link" @click="access('create')">{{ $t('pages.products_details_spare_parts.register_provider') }}</div>
                </div>
            </div>
        </div>
    </div>
...
<script>
import DropdownProvider from './block_provider/DropdownProvider.vue'

export default {
    components: { DropdownProvider },
    props: [ 'providers' ],
    data: function () {
        return {
            display_menu: false,
    ...
    computed: {
        display: function () { return this.$parent.add.provider.display }
    },
    methods: {
        access: function (access) {
            this.$parent.add.provider.display = access
        },
